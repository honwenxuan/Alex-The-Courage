...
public class ShootEnemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            {
                EnemyRagdoll player = hitInfo.collider.GetComponentInParent<EnemyRagdoll>();
                ...
                if (player != null)
                {
                    ...
                    Vector3 forceDirection = player.transform.position - _camera.transform.position;
                    forceDirection.y = 1;
                    forceDirection.Normalize();

                    Vector3 force = forceMagnitude * forceDirection;

                    player.TriggerRagdoll(force, hitInfo.point);
                }
            }
        }
    }
